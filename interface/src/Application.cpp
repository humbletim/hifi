...
#include "assets/AppAssetImporter.h"
...
void Application::onDesktopRootContextCreated(QQmlContext* surfaceContext) {
    ...
    _assetImporter = new AppAssetImporter(engine);
    connect(_fileDownload, &FileScriptingInterface::unzipResult, _assetImporter, &AppAssetImporter::handleUnzip);
...
bool Application::importFromZIP(const QString& filePath) {
    ...
    if (filePath.contains("poly.google.com/downloads")) {
        _assetImporter->addAssetToWorldFromURL(filePath);
    } else {
...
bool Application::importImage(const QString& urlString) {
    ...
    QString filepath(QUrl(urlString).toLocalFile());
    _assetImporter->addAssetToWorld(filepath, "", false, false);
    return true;
}
